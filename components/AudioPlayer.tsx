...
const FALLBACK_COVER =
  'https://images.unsplash.com/photo-1614850523459-c2f4c699c52e?auto=format&fit=crop&q=80&w=300&h=300'
...
export default function AudioPlayer() {
  ...
  const getPlayableUrl = (url: string) => {
    ...
    return url.startsWith('tg://')
      ? `/api/telegram-file?fileId=${url.replace('tg://', '')}&type=audio`
      : url
  }
  ...
  }, [isPlaying, currentChapter?.id, togglePlay])
  ...
  const handleNavigateToText = () => {
    if (safeNovelId && currentChapter?.id) {
  ...
  if (!currentChapter || isClosed) return null
    ...
    <>
      ...
      {isMinimized ? (
        <div className="fixed bottom-6 right-6 z-[60] animate-in slide-in-from-bottom-10 fade-in duration-300">
          <div
            className="relative group w-16 h-16 sm:w-20 sm:h-20 cursor-pointer"
          ...
          >
            ...
            <div className="absolute inset-1 bg-zinc-900 rounded-full flex items-center justify-center overflow-hidden border-2 border-zinc-800 group-hover:border-amber-500 transition-all shadow-2xl">
              <img
                src={safeCover}
                className={`w-full h-full object-cover opacity-50 ${isPlaying ? 'animate-[spin_10s_linear_infinite]' : ''}`}
                alt="cover"
              />
              <span className="absolute text-white font-black text-[9px] sm:text-[10px] opacity-0 group-hover:opacity-100 transition-opacity">
                MỞ
              </span>
            </div>
            ...
            <button
              ...
              className="absolute -top-1 -right-1 w-7 h-7 sm:w-8 sm:h-8 bg-amber-500 text-black rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-90 transition-transform z-10"
            >
              {isPlaying ? (
            ...
            <button
              ...
              className="absolute -top-1 -left-1 w-7 h-7 sm:w-8 sm:h-8 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-90 transition-transform z-10"
              title="Tắt player"
            >
              ✕
            </button>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-[60] select-none pb-2 px-2 sm:px-4 sm:pb-4 animate-in slide-in-from-bottom duration-500">
          <div className="max-w-[1400px] mx-auto bg-[#0a0a0c]/95 backdrop-blur-3xl border border-zinc-800/80 rounded-2xl sm:rounded-[2rem] shadow-[0_-10px_60px_rgba(0,0,0,0.8)] flex flex-col relative">
            <div
              className="w-full h-1.5 sm:h-2 bg-zinc-900 group cursor-pointer relative overflow-hidden sm:rounded-t-[2rem]"
              onClick={handleProgressClick}
            >
              <div
                className="h-full bg-gradient-to-r from-amber-600 to-yellow-400 transition-all duration-150 shadow-lg relative"
            ...
            <div className="flex items-center justify-between px-3 py-3 sm:px-6 sm:py-4 gap-2 sm:gap-6">
              <div
                className="flex items-center gap-3 w-1/4 sm:w-1/3 min-w-0 cursor-pointer group"
                onClick={handleNavigateToText}
              >
                <div className="relative shrink-0 hidden xs:block">
                  <img
                    src={safeCover}
                    className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl object-cover shadow-2xl border border-zinc-800 ${isPlaying ? 'animate-[spin_15s_linear_infinite]' : ''}`}
                    alt=""
                  />
                  {isLoading && (
                  ...
                  {!isLoading && !isError && (
                    <div className="absolute inset-0 rounded-xl bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="text-[9px] font-black text-white uppercase text-center">Đọc</span>
                    </div>
                  )}
                  {isError && (
                    <div className="absolute inset-0 bg-red-900/80 rounded-xl flex items-center justify-center text-xs">
                      ⚠️
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col min-w-0 flex-1 relative h-full justify-center">
                  <p className="text-[9px] sm:text-[10px] text-zinc-500 uppercase font-black truncate group-hover:text-amber-500 transition-colors">
                    {safeNovelTitle}
                  </p>
                  <p className="text-xs sm:text-sm font-bold text-zinc-100 truncate pb-0.5">
                    Tập {currentChapter.chapter_number}: {currentChapter.title}
                  </p>
              ...
              <div className="flex flex-col items-center justify-center gap-1 flex-1 shrink-0">
                <div className="flex items-center gap-3 sm:gap-6">
                  <button
                    onClick={() => skipTime(-15)}
                    className="text-zinc-500 hover:text-amber-500 transition-all hidden md:block active:scale-90"
                    title="Lùi 15s"
                  >
                  ...
                  <button
                    onClick={togglePlay}
                    className={`w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center transition-all shadow-2xl active:scale-95 border-2 sm:border-4 ${
                      isPlaying
                        ? 'bg-zinc-900 border-zinc-800 text-amber-500 hover:border-amber-500'
                        : 'bg-gradient-to-tr from-amber-500 to-yellow-400 border-transparent text-zinc-950 hover:scale-105'
                    }`}
                  >
                    {isLoading ? (
                  ...
                  >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M11.933 12.8a1 1 0 000-1.6L6.6 7.2A1 1 0 005 8v8a1 1 0 001.6.8l5.333-4zM19.933 12.8a1 1 0 000-1.6l-5.334-4A1 1 0 0013 8v8a1 1 0 001.6.8l5.333-4z"
                      />
                    </svg>
                  </button>
                </div>

                <div className="flex items-center gap-2 text-[9px] sm:text-[11px] font-mono text-zinc-500 mt-0.5">
                  <span className="text-zinc-300">{displayCurrentTime}</span>
              ...
              <div className="flex items-center justify-end gap-1.5 sm:gap-3 w-1/4 sm:w-1/3">
                <div className="hidden lg:flex items-center gap-2 bg-zinc-900/80 p-1.5 rounded-full border border-zinc-800 overflow-hidden w-28 hover:w-32 transition-all">
                  <button
                    ...
                    className="text-zinc-400 hover:text-amber-500 ml-1"
                  >
                ...
                <button
                  ...
                  className="p-2 text-zinc-500 hover:text-white hover:bg-red-500 rounded-xl transition-all"
                  title="Tắt player"
                >
                  ✕
                </button>
              ...
              <div className="absolute bottom-full right-4 sm:right-10 w-[280px] bg-zinc-950/98 backdrop-blur-3xl border border-zinc-800 rounded-3xl p-5 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] mb-4 animate-in slide-in-from-bottom-5 fade-in zoom-in-95 z-[70]">
                ...
                <div className="mb-5 pb-5 border-b border-zinc-800/80">
                  <div className="flex items-center justify-between mb-2.5">
                    ...
                    {sleepTimeRemaining && (
                      <span className="text-xs text-red-400 font-mono animate-pulse bg-red-500/10 px-2 py-0.5 rounded-md">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[15, 30, 45, 60].map((m) => (
                      <button
                        ...
                        className="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 py-2.5 rounded-xl text-xs font-bold transition-all border border-zinc-800"
                      >
                        {m} Phút
                      </button>
                    ))}
                    {sleepTimeRemaining && (
                      <button
                        ...
                        className="col-span-2 bg-red-500 text-white py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-red-500/30 mt-1"
                      >
                        HỦY HẸN GIỜ
                      </button>
                    )}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-zinc-100 flex items-center gap-2">
                      <span className="text-amber-500">👁️</span> Đồng bộ văn bản
                    </p>
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full transition-colors relative border ${
                      showText ? 'bg-amber-500 border-amber-400' : 'bg-zinc-900 border-zinc-700'
                    }`}
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-all shadow-md ${
                ...
                <div className="flex justify-between items-center p-5 border-b border-zinc-800/50">
                  <div>
                    ...
                    <p className="text-[10px] text-amber-500 font-bold uppercase tracking-widest mt-0.5">
                      {playlist.length} Tập Sẵn Sàng
                    </p>
                  </div>
                  <button
                    onClick={() => setShowPlaylist(false)}
                    className="w-8 h-8 bg-zinc-900 rounded-full flex items-center justify-center text-zinc-500 hover:bg-red-500 hover:text-white transition-all"
                  >
                    ✕
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-3 space-y-1.5 custom-scrollbar">
                  {playlist.map((chap) => (
                    <button
                      key={chap.id}
                      onClick={() => {
                        useAudioStore.getState().playChapter(chap, playlist)
                        setIsClosed(false)
                        setShowPlaylist(false)
                      }}
                      className={`w-full flex items-center gap-3 p-3 rounded-2xl text-left border transition-all ${
                        chap.id === currentChapter.id
                          ? 'bg-amber-500/10 border-amber-500/30 shadow-inner ring-1 ring-amber-500/20'
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm font-bold truncate leading-tight ${
                            chap.id === currentChapter.id ? 'text-amber-400' : 'text-zinc-300'
                          }`}
                        >
                          {chap.title}
                        </p>
                        {chap.id === currentChapter.id ? (
                          <p className="text-[9px] text-amber-500/80 uppercase font-black tracking-widest mt-1 flex items-center gap-1">
